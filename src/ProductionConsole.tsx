...
import {DEFAULT_API,productionApi,type Contamination,type DataSource,type Health,type IntegrationReadiness,type Journal,type Merchant,type MerchantPayment,type Reconciliation,type Tenant,type TraceReport,type TreasuryPosition,type TrialBalance,type WalletAccount} from './productionApi'

type Tab='readiness'|'treasury'|'ledger'|'settlement'|'merchant'|'trace'
type Snapshot={health:Health;readiness:IntegrationReadiness;contamination:Contamination;treasury:TreasuryPosition[];trial:TrialBalance[];reconciliation:Reconciliation;accounts:WalletAccount[];journals:Journal[];merchants:Merchant[];payments:MerchantPayment[];loadedAt:string}
...
export default function ProductionConsole(){
 ...
 const gate=snapshot&&snapshot.health.status==='ok'&&snapshot.readiness.status==='PASS'&&snapshot.contamination.status==='CLEAN'&&trialBalanced&&snapshot.reconciliation.status==='MATCHED'?'PASS':'BLOCKED'
 ...
 const load=async(id=tenantId,key=secret)=>{
  ...
  try{
   const[health,readiness,contamination,treasury,trial,reconciliation,accounts,journals,merchants,payments]=await Promise.all([
    productionApi.health(base),productionApi.readiness(base,key),productionApi.contamination(base,key,id,source),productionApi.treasury(base,key,id,source),productionApi.trialBalance(base,key,id,source),
   ...
   setSnapshot({health,readiness,contamination,treasury:treasury.positions,trial,reconciliation,accounts,journals,merchants:merchants.data,payments:payments.data,loadedAt:new Date().toISOString()})
