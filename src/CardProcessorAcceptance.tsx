...
export default function CardProcessorAcceptance({tenantId,apiKey,notify}:Props){
 ...
 const run=async()=>{
  ...
  try{
   const customer=await walletBusinessApi.createCardCustomer(tenantId,apiKey,`sprint02-${stamp}`);log(`✓ Customer Mapping · ${customer.id}`)
   const created=await walletBusinessApi.createProcessorCard(tenantId,apiKey,customer.id,`sprint02:${stamp}:create`);setCard(created);log(`✓ CardService → CardProcessor → ${created.provider} · ${created.id}`)
   ...
   const frozen=await walletBusinessApi.freezeProcessorCard(tenantId,apiKey,created.id,`sprint02:${stamp}:freeze`);setCard(frozen);log(`✓ Freeze · ${frozen.status}`)
   const active=await walletBusinessApi.unfreezeProcessorCard(tenantId,apiKey,created.id,`sprint02:${stamp}:unfreeze`);setCard(active);log(`✓ Unfreeze · ${active.status}`)
 ...
 return <section className="wbc-card-acceptance">
  <div className="wbc-card-title"><div><span>SPRINT-02 · THREDD ADAPTER LAYER</span><h3><CreditCard/>Card Processor 验收</h3><p>Customer Mapping → Virtual Card → Retrieve → Freeze → Unfreeze → Balance</p></div><button onClick={()=>void run()} disabled={busy||!tenantId}>{busy?<LoaderCircle className="spin"/>:<Play/>}{busy?'执行中…':'运行卡片完整流'}</button></div>
