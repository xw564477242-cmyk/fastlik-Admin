...
export function runAllScenarios():{state:SimulationState;results:ScenarioResult[];passed:number;failed:number}{
 ...
 run('B-01','租户管理员创建C端用户','Tenant Administrator','B端',()=>({steps:['创建用户 FLU-20001','分配 NovaPay 租户','提交KYC'],checks:[check(actors[1].tenant==='NovaPay Asia','租户范围正确'),check(!actors[1].permissions.includes('treasury:write'),'未获得资金池权限')]}))
 run('B-02','合规员通过KYC','Compliance Officer','B端',()=>({steps:['核验身份资料','制裁筛查通过','KYC升级至L2'],checks:[check(actors[3].permissions.includes('kyc:review'),'具备KYC审核权限')]}))
 ...
 run('B-03','Treasury执行清算','Treasury Operator','B端',()=>{s.tenantFiat-=s.settlementExposure;s.settlementExposure=0;return{steps:['核对授权批次','扣减法币准备金','清算完成'],checks:[check(actors[2].permissions.includes('settlement:execute'),'具备清算权限'),check(s.settlementExposure===0,'敞口已归零'),check(s.tenantFiat===474,'法币准备金与用户卡余额一致')]}})
 ...
 run('B-04','审计员尝试修改资金池','Read Only Auditor','B端',()=>({steps:['请求修改准备金','RBAC拒绝操作'],checks:[check(!actors[5].permissions.includes('treasury:write'),'只读审计员越权被阻止')]}))
