...
export default function Root(){
 ...
 return <div className="root-wrap"><App role={session.role} onLogout={logout} onOpenTenant={setTenantOpen} onOpenWhiteLabel={setWhiteLabelOpen} onOpenSubsystem={()=>setAppsOpen(true)} onOpenTreasury={()=>setTreasuryOpen(true)}/><button className="test-lab-fab" onClick={()=>setTestMode(true)}><FlaskConical/>角色与场景测试</button>{appsOpen&&<div className="apps-overlay"><div className="apps-window"><div className="apps-window-head"><div><b>FastLink 子系统功能应用</b><span>模拟数据 · 可操作业务页面</span></div><button onClick={()=>setAppsOpen(false)}><X/></button></div><div className="apps-window-body"><SubsystemApplications notify={notify}/></div></div></div>}{treasuryOpen&&<TreasuryOperationsCenter onClose={()=>setTreasuryOpen(false)} notify={notify}/>} {tenantOpen&&<TenantOperationsCenter tenantId={tenantOpen} onClose={()=>setTenantOpen(null)} notify={notify}/>} {whiteLabelOpen&&<WhiteLabelDeliveryCenter projectId={whiteLabelOpen} onClose={()=>setWhiteLabelOpen(null)} notify={notify}/>} {toast&&<div className="toast">{toast}</div>}</div>
}
